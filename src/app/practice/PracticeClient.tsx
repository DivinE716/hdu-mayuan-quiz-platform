...
function getPositionKey(typeParam?: string, filterParam?: string): string {
  const mode = filterParam || typeParam || "all";
  return `practice-position-${mode}`;
}
...
export default function PracticeClient({ typeParam, filterParam }: PracticeClientProps) {
  ...
  // ---- 位置同步相关 ----
  const positionKey = getPositionKey(typeParam, filterParam);
  ...
  useEffect(() => {
    ...
    apiGetSetting(positionKey).then((val) => {
      if (val !== null) {
  ...
  const fetchQuestions = useCallback(async () => {
    ...
    try {
      ...
      if (typeParam) {
        params.type = typeParam.split(",");
      }
      if (isAllMode) {
        params.order = "random";
        params.seed = randomSeed.current;
      }
      ...
      if (res.success && res.data) {
        if (res.data.questions.length === 0) {
          ...
          // 恢复保存的位置
          const idx = savedPosition !== null
            ? Math.min(savedPosition, res.data.total - 1)
            : 0;
          setCurrentIndex(idx);
  ...
  }, [typeParam, filterParam, isAllMode, savedPosition]);
  ...
  // ---- 当接近已加载题目的末尾时，自动加载更多 ----
  useEffect(() => {
    if (currentIndex >= questions.length - 10 && hasMore && !loading) {
      loadMore();
    }
  ...
  const correctAnswers: string[] = (() => {
    if (!currentQuestion) return [];
    try {
      return JSON.parse(currentQuestion.answer) as string[];
    } catch {
      return [];
    }
  })();
    ...
    syncProgress(currentQuestion.id, [
      {
        ...
        action: newFav ? "favorite" : "unfavorite",
      },
    ]);
  };
  ...
  const goTo = (index: number) => {
    ...
    setShowAnalysis(false);
    // 异步保存到服务器（静默）
    apiSetSetting(positionKey, String(index));
  };
  ...
  function getOptionStyle(key: string): string {
    ...
    if (isCorrectOption) {
      return "border-morandi-success bg-morandi-success/15 ring-2 ring-morandi-success/40";
    }
    if (isUserSelected && !isCorrectOption) {
      return "border-morandi-danger bg-morandi-danger/10 ring-2 ring-morandi-danger/40";
    }
  ...
  const modeLabel = filterParam === "wrong"
    ? "错题本"
    : filterParam === "favorite"
      ? "收藏夹"
      : typeParam
        ? { single: "单选题", multiple: "多选题", judge: "判断题" }[typeParam] ?? "刷题"
        : "全部题目";
      ...
      <main className="max-w-2xl mx-auto px-4 py-6 pb-24">
        ...
        <div className="flex items-center justify-between mb-5">
          <AppLink
            href="/"
            replace
            className="text-sm text-morandi-text-soft hover:text-morandi-primary transition-colors flex items-center gap-1 cursor-pointer"
          >
            ← 返回
          </AppLink>
          <span className="text-sm text-morandi-text-soft font-medium">
            {modeLabel}
            {totalQuestions > 0 && (
              <span className="ml-1 text-morandi-primary">
        ...
        {!loading && error && (
          <div className="flex flex-col items-center gap-4 py-20">
            ...
            <AppLink
              href="/"
              replace
              className="text-morandi-primary hover:underline text-sm cursor-pointer"
            >
              返回首页
            </AppLink>
          </div>
        ...
        {!loading && currentQuestion && (
          <div className="space-y-4">
              ...
              <button
                ...
                className={`text-2xl transition-all active:scale-125 ${
                  currentState?.isFavorite
                    ...
                    : "text-morandi-muted hover:text-yellow-400"
                }`}
                title={currentState?.isFavorite ? "取消收藏" : "收藏"}
              >
                ★
              </button>
            </div>
            ...
            <div className="bg-morandi-surface rounded-2xl p-6 shadow-sm border border-morandi-muted/60">
              <p className="text-base leading-relaxed text-morandi-text-main">
              ...
              {options.map((opt) => (
                <button
                  ...
                  onClick={() => {
                    if (currentQuestion.type === "multiple") {
                      ...
                      handleSingleChoice(opt.key);
                    }
                  }}
                  disabled={isAnswered && currentQuestion.type !== "multiple"}
                  className={`w-full text-left px-5 py-3.5 rounded-xl border-2
                ...
                >
                  <span className="inline-flex items-center gap-3">
                    {currentQuestion.type === "multiple" ? (
                      <span
                        className={`w-5 h-5 rounded border-2 flex-shrink-0 flex items-center justify-center text-xs
                                    transition-all ${
                                      currentState?.selectedOptions.includes(opt.key)
                                        ? "bg-morandi-primary border-morandi-primary text-white"
                                        : "border-morandi-muted"
                                    } ${
                                      isAnswered && correctAnswers.includes(opt.key)
                                        ? "!bg-morandi-success !border-morandi-success text-white"
                                        : ""
                                    }`}
                      >
                      ...
                      <span
                        className={`w-6 h-6 rounded-full border-2 flex-shrink-0 flex items-center justify-center text-xs font-bold
                                    transition-all ${
                                      isAnswered && correctAnswers.includes(opt.key)
                                        ? "bg-morandi-success border-morandi-success text-white"
                                        : isAnswered && currentState?.selectedOptions.includes(opt.key) && !correctAnswers.includes(opt.key)
                                          ? "bg-morandi-danger border-morandi-danger text-white"
                                          : "border-morandi-muted text-transparent"
                                    }`}
                      >
                    ...
                    <span>
                      <span className="font-medium text-morandi-primary mr-2">
                        {opt.key}
                      </span>
                      {opt.text}
                    </span>
            ...
            {currentQuestion.type === "multiple" && !isAnswered && (
              <button
                ...
                disabled={currentState?.selectedOptions.length === 0}
                className="w-full py-3 rounded-xl bg-morandi-primary text-white font-medium
                           text-base hover:opacity-90 hover:shadow-md
              ...
              <div
                className={`rounded-xl p-4 text-sm leading-relaxed ${
                  currentState?.answerState === "correct"
                    ? "bg-morandi-success/10 border border-morandi-success/30 text-morandi-success"
                    : "bg-morandi-danger/10 border border-morandi-danger/30 text-morandi-danger"
                }`}
              >
                <p className="font-semibold mb-1">
                  {currentState?.answerState === "correct" ? "✅ 回答正确！" : "❌ 回答错误"}
                </p>
                {currentState?.answerState === "wrong" && (
                  <p>
                    正确答案：{correctAnswers.join("、")}
                    {currentQuestion.type === "judge" &&
                      `（${correctAnswers[0] === "A" ? "正确" : "错误"}）`}
                  </p>
                )}
              </div>
            )}
            ...
            {isAnswered && showAnalysis && (
              <div className="bg-morandi-surface rounded-2xl p-5 shadow-sm border border-morandi-muted/60">
                <h4 className="text-sm font-semibold text-morandi-text-main mb-2">
                  📖 题目解析
                </h4>
                <p className="text-sm leading-relaxed text-morandi-text-soft">
                  {currentQuestion.analysisText || "暂无解析内容"}
                </p>
              </div>
            )}
            ...
            <div className="flex items-center gap-3 pt-2">
              <button
                ...
                className="flex-1 py-2.5 rounded-xl border-2 border-morandi-muted
                           text-sm font-medium text-morandi-text-main
                           hover:bg-morandi-surface disabled:opacity-30 disabled:cursor-not-allowed
                           transition-all"
              >
                上一题
              </button>
              ...
              {isAnswered && !showAnalysis && (
                <button
                  ...
                  className="flex-1 py-2.5 rounded-xl bg-morandi-secondary/50
                             text-sm font-medium text-morandi-text-main
                             hover:bg-morandi-secondary/70 transition-all"
                >
                  查看解析
                </button>
              )}
              ...
              <button
                ...
                className="flex-1 py-2.5 rounded-xl bg-morandi-primary text-white
                           text-sm font-medium hover:opacity-90 hover:shadow-md
